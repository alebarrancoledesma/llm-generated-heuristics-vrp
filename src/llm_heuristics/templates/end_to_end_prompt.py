...
END_TO_END_PROMPT = Template("""
<problem-description>
You are a highly-skilled professor in AI planning searching a plan for a PDDL task from the domain <domain>$name</domain>. You will be given the PDDL domain and the PDDL instance, and you need to return the plan in the format shown below. Next, you will receive a sequence of examples for your task and finally the definition of the $name domain.
</problem-description>
...
3) The actions in the plan must be from the set of actions in the domain $name, that is, they must use the same name and same number of parameters as one of the action schemas.
